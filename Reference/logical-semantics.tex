...
\begin{newver}
and starting with Version 2.7, the use of functions as first-class values.
...
in Sections~\ref{sec:core} and~\ref{sec:ho-core}}.
...
\item
\new{an infinite set $\sop$ of \define{sort parameters} $u$,}
...
%
%%CT Reintroduced \st \te \lo because they are needed for theories and logics
\item
...
An abstract syntax for monomorphic sorts $\sigma$ \new{and polymorphic sorts $\tau$},
...
a \define{map abstraction binder}\index{binder!lambda binder}
for $x$;}
...
A \define{monomorphic sort substitution} is a sort substitution that maps every parameter
...
\new{We write  $f{:}\tau_1\cdots\tau_n\tau \in \Sigma$
to denote that function $f$ has some rank $\hat\tau_1\cdots\hat\tau_n\hat\tau$ in $\Sigma$, and
...
%%CT Removed because not needed anymore
...
With this definition, it is possible to show that every \new{closed} term has 
at most one sort in a given signature $\Sigma$.\endnote{
  ...
  symbols $f^\tau$, without the second conjunct of the rule's side condition.  
  This would allow formation of terms with annotated
  function symbols $f^\tau$, even when $f$ lacked two ranks of the
  forms $\tau_1 \cdots \tau_k\tau$ and $\tau_1 \cdots
  \tau_k\tau'$, for distinct $\tau$ and $\tau'$.
  The rationale for keeping this second conjunct is that with it, function
...
\begin{newver}
For interpreting $\fconstruct$ sorts, we will use the notion of a \define{map}, i.e., a functional binary relation.
...
\new{where Case~\ref{ruleI} applies to polymorphic terms
and the other cases apply only to monomorphic terms}.
...
in the concrete syntax of theory declarations with the binder \ter{par}.
%CT addressed
%\cbrem{or with the declare-sort-parameter command?}
}
\end{newver}
